...
internal class VariableResolver(IServiceProvider serviceProvider) : IVariableResolver
{
        ...
        bool recursive = false)
    {
        ...
        return ReplaceVariables(value, wrappedDict, variablesToIgnore, recursive);
    }
    ...
    {
        return !string.IsNullOrEmpty(name) && name.IndexOf('.') == -1;
    }

    private bool TryGetSpecialFolder(string name, out string? value)
    {
    ...
    private bool TryGetSpecialFolderHelper(string name, out string? value)
    {
        value = null;
        try
        {
